...
class Network:
	...
	def __init__(self, N=100, theta=0.2, f=0.5, lmbda=0.9, gamma=0.5, initial_mean=1, initial_variance=1): # Not very beautiful ...
		self.N = N
	...
	def update_states(self, xi=np.zeros(1)):
		H0 = (self.N-1)*(self.f*self.W.mean() - self.theta/(self.N-1))
		H1 = self.f*self.gamma*math.sqrt(self.N-1)
		inhibition = H0 + H1*xi.sum()/(self.f*self.N) + self.lmbda*(self.s.sum() - self.f*self.N)
		self.vi = np.dot(self.W, self.s) + self.gamma*math.sqrt(self.N)*xi - inhibition
	...
	def update_weights(self, learn_rate, epsilon):
		...
		coeffs = - (self.vi < self.theta) + (self.vi <= theta0) + (self.vi > self.theta) - (self.vi >= theta1)
